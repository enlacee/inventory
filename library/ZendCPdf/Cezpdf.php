...
require_once 'ZendCPdf/class.pdf.php';

class ZendCPdf_Cezpdf extends Cpdf {
//==============================================================================
...
function ZendCPdf_Cezpdf($paper='a4',$orientation='portrait', $titulo='Titulo de Reporte',$ide=0,$usuario=''){
	...
	if($orientation == 'portrait') {
		$this->ezImage("images/iconos/stack_urbano.jpg",-15,30,5, 'left');
		$this->addText(50,820,12, "Sistema de Inventarios");
		
		//$this->reopenObject($id);
		$x = $this->getTextWidth(12,$titulo);
		
		$this->addText(578 - $x,820,12,$titulo);
		$this->line(50,818,578,818);
		
		$this->ezSetMargins(50,70,50,50);
		$this->ezStartPageNumbers(578,28,8,'','{PAGENUM} de {TOTALPAGENUM}',1);
		$this->line(20,40,578,40);
		$this->addText(20,32,8,'Impreso el ' . date('d/m/Y h:i:s a').($usuario==''?'':' - Usuario:'.$usuario));
		$this->addText(20,22,8,'Importaciones Inga - http://www.importacionesinga.com.pe');
	}
	else {
		$this->ezStartPageNumbers(750,28,8,'','{PAGENUM}',1);
		$this->line(20,40,800,40);
		$this->addText(50,32,8,'Impreso el ' . date('d/m/Y h:i:s a'));
		$this->addText(50,22,8,'Consultas de Saldos y Movimientos - http://www.cajasipan.com.pe');
	}
	$this->restoreState();
	$this->closeObject();
...
function ezTable(&$data,$cols='',$title='',$options='', $filtro=''){
  ...
  foreach($data as $row){
	...
	//SUMADOR
	if($this->ide==1){
		$this->sum1 = $this->sum1 + $row['total_venta'];
		if($row['moneda']=='SOLES'){
			$this->sum2 = $this->sum2 + $row['total_venta'];
		}else{
			$this->sum3 = $this->sum3 + $row['total_venta'];	
		}		
		$this->con1 = $this->con1 + 1;
		if($row['moneda']=='SOLES'){
			$this->con2 = $this->con2 + 1;
		}else{
			$this->con3 = $this->con3 + 1;
		}
	}
    foreach($cols as $colName=>$colHeading){
  ...
  if (strlen($title)){
    ...
    $this->addText($middle-$w/2,$this->y+15,$options['titleFontSize'],$title);
	
	if($filtro<>''){
		$w = $this->getTextWidth($options['titleFontSize'],$filtro);
		$this->addText($middle-$w/2,$this->y,$options['titleFontSize'],$filtro);
	}
...
//        $x1=$x1+$dm;
      ...
      foreach($cols as $colName=>$colTitle){
        ...
        foreach ($lines as $line){
			switch($colName){
			case "ctap_mone" :
				$line = ($line=='01'?'SOLES':'DOLARES');
			break;
			case "ctap_nume" :
				$line = substr($line,0,3).'-'.substr($line,3,3).'-'.substr($line,6,5).'-'.substr($line,11,1);
			...
			break;	
			case "movi_fech" :
				$line = date("d/m/Y",strtotime($line));
				break;	
			case "deud_fven" :
				$line = date("d/m/Y",strtotime($line));
				break;
