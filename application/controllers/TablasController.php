...
class TablasController extends Zend_Controller_Action
{
	...
	public function bancoListaAction(){
        $this->_helper->viewRenderer->setNoRender();
        $rs = $this->objDatos->sp_bancos_lista($_POST);
		$data = $rs;
		echo json_encode($data);
	}
	...
	public function cuentaBancoListaAction(){
        $this->_helper->viewRenderer->setNoRender();
		$banco = $_POST['banco'];
		$query = "SELECT cta_id,nro_cta FROM v_banco WHERE ban_id='$banco'";
		$result = $this->objDatos->sp_obtenerdatasql($query);
