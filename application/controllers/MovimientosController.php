...
class MovimientosController extends Zend_Controller_Action{
	...
	public function condicionPagoAction(){
        $this->_helper->viewRenderer->setNoRender();
		$result = $this->objDatos->getCondicionPago($_POST);
		echo json_encode($result);
	}	
	
	public function saldoPendienteClienteActualizarAction(){
        $_POST['tienda'] = $this->sessName->se_age_id;
        $this->_helper->viewRenderer->setNoRender();
		$result = $this->objDatos->setSaldoPendienteCanje($_POST);
	...
	public function saldoPendienteClienteAction(){
        $this->_helper->viewRenderer->setNoRender();
        $_POST['tienda'] = $this->sessName->se_age_id;
		$rsData = $this->objDatos->sp_saldo_pendiente_cliente($_POST);  
...
where dve.mve_id =".$_POST['txtpar2']);
	...
	public function canjeListaImpresionAction(){
		ini_set('memory_limit', '512M'); //Raise to 512 MB 
		ini_set('max_execution_time', '10000'); //Raise to 512 MB
        $this->_helper->viewRenderer->setNoRender();

		//$pdf = new ZendCPdf_Cezpdf();
		$pdf = new ZendCPdf_Cezpdf('a4', 'portrait','Listado de Canjes de Ventas',0,$this->sessName->se_nombres.' '.$this->sessName->se_apellidos);
		$col_names = array(
			'fecha_canje' => 'Fecha',
			'canje_codigo' => utf8_decode('Código'),
			'persona' => 'Persona',
			'moneda' => 'Moneda',
			'condicion' => utf8_decode('Condición'),
			'total_canje' => 'Total Canje',
		);

		$options = array(
			'width' => 550,
			'cols' => array(
				'fecha_canje' => array('justification'=>'center'),
				'canje_codigo' => array('justification'=>'center'),
				'persona' => array('justification'=>'left'),
				'moneda' => array('justification'=>'left'),
				'condicion' => array('justification'=>'right'),
				'total_canje' => array('justification'=>'right'),
			)
		);

		$_POST['age_id'] = $this->sessName->se_age_id;
		$db_data = $this->objDatos->sp_canje_lista($_POST,2);
		$pdf->ezTable($db_data, $col_names, 'Listado de Canjes de Ventas', $options);
		$pdf->ezStream();
    }
