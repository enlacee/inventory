...
where dve.mve_id =".$_POST['txtpar2']);
		...
		if($_POST['tipo'] == '1'){	
			...
			if($rsData[0]['doc_id']==2){	//FACTURA
				...
				$page->drawText($rsData[0]['nombre_cliente'], 90 ,402);			
				$page->drawText(trim($rsData[0]['direccion']).' - '.trim($rsData[0]['ubigeo']), 90 ,392);
				...
				foreach($rsDetalle as $solo){	
					...
					$page->drawText(utf8_decode($solo['cantidad']), 85 - $lon_tot - 30 ,$y)  //45
						->drawText(utf8_decode($solo['codigo1']), 80 ,$y)			
						->drawText(utf8_decode($solo['producto']), 165 ,$y)												  
						->drawText(utf8_decode($solo['marca']), 420 ,$y);												  
				...
				$page->setFont($font, 7)
					...
					->drawText($this->num2letras($rsData[0]['total_venta']), 40 ,220);
			...
			if($rsData[0]['doc_id']==3){	 //BOLETA
				...
				$page->drawText($rsData[0]['nombre_cliente'], 90 ,385);			
				$page->drawText(trim($rsData[0]['direccion']).' - '.trim($rsData[0]['ubigeo']), 90 ,375);
				...
				foreach($rsDetalle as $solo){	
					...
					$page->drawText(utf8_decode($solo['cantidad']), 85 - $lon_tot - 30 ,$y)  //45
						->drawText(utf8_decode($solo['codigo1']), 80 ,$y)			
						->drawText(utf8_decode($solo['producto']), 142 ,$y)												  
						->drawText(utf8_decode($solo['marca']), 435 ,$y);												  
					$lon_tot = $this->getTextWidth(utf8_decode($solo['precio_venta']), $page->getFont(), $page->getFontSize());
