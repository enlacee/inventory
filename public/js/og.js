...
var vtindex = 0;
var vtlist = [];
og.hostName='http://localhost:8080';
og.eventTimeouts = [];
og.otherData = [];
cit.minListWidth = 300;
og.pageSize = 10;
...
og.cadenaGrid  = function(stores,extra){    
    var m = stores.getModifiedRecords();
    	var cad = '';
        if(m.length != 0 || extra){   
            //alert(m.length);
            for(i=0;i<stores.getCount();i++){
                var rec = stores.getAt(i);
           		...
           		$.each(rec.data, function(index, value) { 
           		   //alert(index+value)
                	cad = cad+index + '::'+value+",,";    
...
og.cadenaGridCampo  = function(stores,extra,campo){    
    var m = stores.getModifiedRecords();
    	var cad = '';
        if(m.length != 0 || extra){   
            //alert(m.length);
            for(i=0;i<stores.getCount();i++){
                var rec = stores.getAt(i);
           		...
           		$.each(rec.data, function(index, value) { 
           		   //alert(index+value)
                           if(campo==index || index=='BD_ID' || index=='GUIENV')
                            cad = cad+index + '::'+value+",,";    
...
og.recorreForm = function(formItems,item,action){    
     formItems.each(function(f){
...
// functions
og.fechaRender = function(v, params, record){
    var values = v+'cadena';
    var exitr = values.indexOf('/');
	var exits = values.indexOf('-');
    if(exitr>0)
            var dato;
    else{
...
og.getUrl = function(controller, action, args,module) {
	//var url = og.hostName;
        var url = '';
	url += "/" + module;
	url += "/" + controller;
...
og.openLink = function(url, options) {
	...
	Ext.Ajax.request({
		...
		callback: function(options, success, response) {
			og.hideLoading(); 
			//alert(options+'--'+success+'--'+ response)
			if (success) {
...
og.submit = function(form, options) {
	...
	var url = og.makeAjaxUrl(form.action).replace(/ajax\=true/g, "upload=true");
	form.action = url;
	og.loading();
...
og.processResponse = function(data, options, url) {
	...
	// alert(data.errorCode+options.options.actions)
	    if(data.errorCode==0 && options.options.actions){
	      ur.actionsConfig(data.cod,data.tabla); 
	   }
